...
export default function BoutiqueDetailScreen() {
  ...
  // 2. Partager directement sur WhatsApp (ou autres apps sociales)
  const shareToWhatsApp = async () => {
    if (qrRef.current) {
      qrRef.current.toDataURL(async (dataURL: string) => {
        const base64 = dataURL.replace(/^data:image\/png;base64,/, '');
        const fileUri = FileSystem.cacheDirectory + `share-qr.png`;
        await FileSystem.writeAsStringAsync(fileUri, base64, { encoding: FileSystem.EncodingType.Base64 });

        // Partage direct. WhatsApp apparaîtra dans la liste des applications
        await Sharing.shareAsync(fileUri, {
          mimeType: 'image/png',
  ...
  const copyStoreLink = async () => {
    ...
    // Ou en prod : https://uzisha.com/boutique/ID_DE_LA_BOUTIQUE
    const url = Linking.createURL(`boutique/${boutique.id}`);
      ...
      >
        ...
        >
          <Ionicons
            ...
            size={18}
            color={likedItems.includes(item.id) ? "#FF2D55" : "#FFF"}
          />
        </TouchableOpacity>
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? Colors.black : Colors.white }]}>
      ...
      {/* HEADER PARALLAXE IMMERSIF */}
      <Animated.View style={[styles.header, { transform: [{ translateY: headerTranslateY }] }]}>
        <Animated.Image
          source={{ uri: boutique?.image }}
          style={[styles.headerBg, { opacity: imageOpacity, transform: [{ scale: imageScale }] }]}
        />
        <LinearGradient
          ...
          <Animated.View style={[
            ...
            <View style={styles.qrContainer}>
              <View style={styles.qrWrapper}>
                <QRCode
                  key={isShareModalVisible ? 'visible' : 'hidden'} // Force le re-render
                  value={`boutique://${boutique.id}`}
                  size={180}
      ...
      <Animated.View style={[styles.fab, {
        ...
        <TouchableOpacity onPress={() => setIsConciergeOpen(true)}>
